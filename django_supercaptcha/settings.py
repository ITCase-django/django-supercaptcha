...
HTML_TEMPLATE = getattr(
    ...
    """
    <img src="%(src)s?%(rnd)s" width="%(width)s" height="%(height)s"
...
HTML_TEMPLATE_WITH_REFRESH = getattr(
    ...
    <script>
        function refreshCaptcha() {
            ...
            if (img.nodeType==document.TEXT_NODE) {
    ...
    <img src="%(src)s?%(rnd)s" width="%(width)s" height="%(height)s"
         alt="%(alt)s" title="%(alt)s" />
    <a onclick="refreshCaptcha()" href="#refresh"
       class="supercaptcha-refresh">%(refresh_text)s</a>
    <input%(input_attrs)s maxlength="%(length)s" />
    """
)
